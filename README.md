...
4. Inside `manuscript_section/`, create a folder called `rdata/` and set all chunks in `manuscript_sections/stan_model_fitting.Rmd` to `eval = TRUE`. (This will write some `.rds` files to the `rdata` folder.)
5. Knit `noisy_speech_manuscript.Rmd`. (If you want to knit the manuscript again, you can set the chunks back to `eval = FALSE` in order to save time.)
